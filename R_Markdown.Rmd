...
for (i in states2) {
  state_subgroup_bar[[i]] = ggplot(acgr_long_subgroups %>% filter(statename == i|statename == "United States"), aes(x = race, y = acgr, fill = National_avg)) + geom_bar(position = "dodge", stat = "identity") + theme_bw() + xlab("Subgroup") + ylab("ACGR") + ggtitle(i) + coord_flip()
